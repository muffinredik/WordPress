...
function hpat_meta_boxes_init() {
    ...
    add_meta_box( 'hpat-task-meta', __('Task Information', 'hopper-projects-and-tasks'), 
...
function hpat_task_meta_box( $post, $box ) {
    ...
    echo '<p>' . __('Done', 'hopper-projects-and-tasks') . ': <input type="checkbox" name="hpat_task_state"';
    if( $hpat_task_state == 'on' ){ 
        echo 'checked '; 
    }
    echo '/></p>'; 
    echo '<p>' . __('Project', 'hopper-projects-and-tasks') . ': <select name="hpat_project_slug">';
    foreach ($hpat_projects as $hpat_project) {
        echo '<option value="' . $hpat_project['slug'] . '" ' . selected( $hpat_project_slug, $hpat_project['slug'], false ) . '>' 
                . $hpat_project['title'] . 
            '</option>';
    }
    echo '</select>';
